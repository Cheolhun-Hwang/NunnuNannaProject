...
import com.hooney.nunnunannaproject.fragments.CameraDetectFragment;
...
import com.hooney.nunnunannaproject.utils.TTS;

public class TestEyeActivity extends AppCompatActivity {
    private final String TAG = TestEyeActivity.class.getSimpleName();
    private CameraDetectFragment cameraDetectFragment;

    private Thread testEnvCheckerThread;
    private boolean isEnvChecker;
    ...
    private void initView() {
        ...
        findViewById(R.id.next_scroll).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!tts.isRunning() && (step>0) ){
                    if(testFlag == 1){
                        if(step == 23){
                            saveDb();
                            if(type != 3){
                                Intent intent = new Intent(getApplicationContext(), TestEyeActivity.class);
                                intent.putExtra("test_type", type+=1);
                                intent.putExtra("test_code", code);
                                startActivity(intent);
                            }
                            finish();
                        }else{
                            testFlag = 2;
                            horizontalScrollView.scrollTo(328, 0);
                            step += 10;
                        }
                    }else if(testFlag == 2){
                        testFlag = 3;
                        ...
                        testFlag = 1;
                        horizontalScrollView.scrollTo(660, 0);
                        step += 10;
                    }
                }
            }
        ...
        findViewById(R.id.stop_test).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder alert = new AlertDialog.Builder(TestEyeActivity.this);
                alert.setTitle("정말 취소하시겠습니까?");
                alert.setMessage("취소하시면 지금까지 진행하신\n검사결과가 사라집니다.");
                alert.setPositiveButton("취소 합니다.", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        deleteBeforeTest();
                    }
                }).setNegativeButton("계속 진행합니다.", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                    }
    ...
    private void saveDb() {
        ...
        Log.i(TAG, "DB result : " + s_result);
        Log.i(TAG, "DB code : " + code);
        Log.i(TAG, "DB date : " + DateUtil.getDate());
        Log.i(TAG, "DB type : " + type);
        Log.i(TAG, "DB cnt : " + cnt);
        ResultData item = new ResultData();
        item.setCnt(cnt);
        item.setCode(code);
        item.setDate(DateUtil.getDate());
        item.setResult(s_result);
        item.setType(type);
        resultViewModel.insert(item);
    }
    ...
    private void init(Bundle savedInstanceState) {
        ...
        type = getIntent().getIntExtra("test_type", 0);
        step = 0;

        tts = new TTS(TestEyeActivity.this);

        resultViewModel = ViewModelProviders.of(TestEyeActivity.this).get(ResultViewModel.class);

        cameraDetectFragment = new CameraDetectFragment();
        if (null == savedInstanceState) {
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.container, cameraDetectFragment).commit();
        }

        testEnvCheckerThread = initTestEnvCheckerThred();
        testEnvCheckerThread.start();
    }

    private Thread initTestEnvCheckerThred(){
        return new Thread(new Runnable() {
            @Override
            public void run() {
                while(isEnvChecker){
                    if(cameraDetectFragment != null){
                        Log.i(TAG, "Test ENV Check... : " + cameraDetectFragment.isTestEnvCheck());
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                final Vibrator vibrator = (Vibrator)getSystemService(Context.VIBRATOR_SERVICE);
                                if(tts.isTTS()){
                                    if(!cameraDetectFragment.isTestEnvCheck()){
                                        if(cameraDetectFragment.getAvg_dist() > 0.54){
                                            tts.speech("핸드폰을 좀 더 멀리 위치시키고 중심점을 주시하세요");
                                            vibrator.vibrate(500);
                                        }else if(cameraDetectFragment.getAvg_dist() < 0.36){
                                            tts.speech("핸드폰을 좀 더 가까이 위치시키고 중심점을 주시하세요");
                                            vibrator.vibrate(500);
                                        }
                                    }else{
                                        if(!tts.isRunning() && cameraDetectFragment.isTestEnvCheck()){
                                            startStep();
                                        }
                                    }
                                }
    ...
    private void startStep(){
        Log.i(TAG, "type : " + type + " /step : " + step);
        if(type == 0 && step == 0){
            tts.speech("검사를 시작하겠습니다. 총 네 번의 검사가 진행됩니다. 먼저 왼쪽 눈을 감으세요.\n" +
                    "오른쪽 눈으로 가운데 원을 계속 주시한 상태로, 어두운 부분이 있다면 칸을 터치하세요.\n" +
                    "완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 0 && step == 11){
            tts.speech("오른쪽의 원을 계속 주시한 상태로, 어두운 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 0 && step == 22){
            tts.speech("왼쪽의 원을 계속 주시한 상태로, 어두운 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 1 && step == 0){
            tts.speech("두번째 검사를 시작하겠습니다. 오른쪽 눈을 감으세요.\n" +
                    "왼쪽 눈으로 가운데 원을 계속 주시한 상태로, 어두운 부분이 있다면 칸을 터치하세요.\n" +
                    "완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 1 && step == 11){
            tts.speech("오른쪽의 원을 계속 주시한 상태로, 어두운 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 1 && step == 22){
            tts.speech("왼쪽의 원을 계속 주시한 상태로, 어두운 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 2 && step == 0){
            tts.speech("세번째 검사를 시작하겠습니다. 왼쪽 눈을 감으세요.\n" +
                    "오른쪽 눈으로 가운데 원을 계속 주시한 상태로, 휘어진 부분이 있다면 칸을 터치하세요.\n" +
                    "완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 2 && step == 11){
            tts.speech("오른쪽의 원을 계속 주시한 상태로, 휘어진 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 2 && step == 22){
            tts.speech("왼쪽의 원을 계속 주시한 상태로, 휘어진 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 3 && step == 0){
            tts.speech("마지막 검사를 시작하겠습니다. 오른쪽 눈을 감으세요.\n" +
                    "왼쪽 눈으로 가운데 원을 계속 주시한 상태로, 휘어진 부분이 있다면 칸을 터치하세요.\n" +
                    "완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 3 && step == 11){
            tts.speech("오른쪽의 원을 계속 주시한 상태로, 휘어진 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }else if(type == 3 && step == 22){
            tts.speech("왼쪽의 원을 계속 주시한 상태로, 휘어진 부분이 있다면 칸을 터치하세요.\n완료시 다음 단계 누르세요.");
            step+=1;
        }
    }
    ...
    private void selectIndex(int x, int y, View view){
        if (testGrid[x][y]){
            ...
            if(type == 0 || type == 1){
                //find black > grey
                view.setBackground(getDrawable(R.drawable.test_grid_true_grey_layout));
